...
public class MyMusicUtils {
    ...
    public static File getAudioRecordDir(){
        //把String再转换为一个file对象
        File audioRecordFile = new File(MyMusicUtils.getSdCardAbsolutePath() + AUDIO_RECORD);
    ...
    public static File getDownAudioRecordDir(){
        File audioRecordFile = new File(MyMusicUtils.getSdCardAbsolutePath() + DOWNLOAD_AUDIO_RECORD);
        if (!audioRecordFile.exists()) {
            // 此处可能会创建失败，暂不考虑
            audioRecordFile.mkdir();
        }
